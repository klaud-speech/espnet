...
train_set=tr_adult01     # Name of training set.
valid_set=dt_adult01       # Name of validation set used for monitoring/tuning network training
test_sets=et_adult01       # Names of test sets. Multiple items (e.g., both dev and eval sets) can be specified.
srctexts=data/tr_adult01/text        # Used for the training of BPE and LM and the creation of a vocabulary list.
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        log "gpu_inference =${gpu_inference}"
        ...
        for dset in ${test_sets}; do
            ...
            for f in token token_int score text; do
                ...
                done | LC_ALL=ko_KR.utf-8 sort -k1 >"${_dir}/${f}"
            done
        done
    fi
