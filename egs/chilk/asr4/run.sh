...
lang=elderly  #child #adult01  # de, en, es, fr, it, nl, pt, ru
...
#    selected=/data/home/data_8k
    selected=/data/home/data_8k/${lang}/extracted
...
if [ ${stage} -le 2 ]; then
 ...
 #   text2token.py -s 1 -n 1 data/dt_${lang}/text | cut -f 2- -d" " | tr " " "\n" \
 #   | sort | uniq | grep -v -e '^\s*$' | awk '{print $0 " " NR+1}' >> ${dict}
    wc -l ${dict}
